...
close all;
FILE = {'./exp04_present_exp01v2'};
fg = 1;

figure(fg); fg = fg + 1;
dat = dlmread(FILE{1});
for j = 2:size(dat, 1)
    ln(j-1) = line(dat(1,:), dat(j,:),'Marker','o','LineWidth',2,'MarkerFaceColor','none','MarkerSize',8,'Color',[j/size(dat, 1) 0 0]);
end
set(gca, 'yScale', 'log', 'yMinorTick','on');
L = legend('Near-end user', 'Far-end user');
