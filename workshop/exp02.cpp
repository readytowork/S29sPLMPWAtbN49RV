...
int main()
{
  ...
  QAM qpsk(16);                     //The QPSK modulator class
  ...
  N0 = 0;
  dist_1 = 20;                   //Distance form transmitter to receiver 1 (meter)
  dist_2 = 100;                  //                   *                  2
  h1 = pow(dist_1, -4.5);        //Channel gain for receiver 1
  h2 = pow(dist_2, -4.5);        //
  cout << "h1 = " << h1 << " h2 = " << h2 << endl;
  alpha = linspace(0.0, 0.0, 1);//Simulate for different weight on power
...
//  N0 = Eb * pow(EbN0, -1.0);     //N0 is the variance of the (complex valued) noise.
  Number_of_bits = 100;       //One hundred thousand bits is transmitted for each Eb/N0 value
//  alpha = 0.8;                   //Ratio of allocated power
  ...
  for (i = 0; i < alpha.length(); i++) {
    ...
    cout << transmitted_symbols_2 << endl << endl;
    ...
    cout << transmitted_symbols << endl << endl;
    ...
    cout << received_symbols_2 << endl << endl;
    ...
    if (alpha(i) > (1-alpha(i))) {
      ...
      //Demodulate the received QPSK symbols into received bits: Layer 1
      received_bits_2 = qpsk.demodulate_bits(received_symbols_2 / pow(h2, 0.5) / pow(Ps, 0.5));
